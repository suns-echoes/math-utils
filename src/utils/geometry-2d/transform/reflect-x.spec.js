...
describe('Reflect point x coordinate', () => {
	...
	it('reflects over (10,10)', () => {
		const { x, y } = reflectX({ x: 10, y: 10 }, { x: 10, y: 10 });
	...
	it('reflects over (5,5))', () => {
		const { x, y } = reflectX({ x: 10, y: 10 }, { x: 5, y: 5 });
	...
	it('reflects over (20,20))', () => {
		const { x, y } = reflectX({ x: 10, y: 10 }, { x: 20, y: 20 });
